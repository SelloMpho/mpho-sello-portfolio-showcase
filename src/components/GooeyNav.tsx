...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  return (
    <>
      <style>
        {`
          :root {
            ...
            --color-2: hsl(var(--accent));
            --color-3: hsl(var(--primary-dark));
            --color-4: hsl(var(--ring));
      ...
      <div ref={containerRef} className="gooey-nav-container relative">
        <nav>
          <ul ref={navRef} className="flex items-center gap-1 list-none m-0 p-0 relative z-[2]">
            {items.map((item, index) => (
              <li
                key={item.href}
                className={`relative cursor-pointer text-sm font-medium transition-colors duration-300 ${
                  activeIndex === index ? 'active text-primary-foreground' : 'text-foreground'
                }`}
              >
                <a
                  href={item.href}
                  ...
                  className="outline-none py-[0.5em] px-[0.8em] inline-block"
                >
                  {item.label}
                </a>
              </li>
